...
REPO_PATH = os.path.join(ROOT_DIR, "FreqNet-DeepfakeDetection")
CKPT_PATH = os.path.join(REPO_PATH, "checkpoints/pretrained_freqnet.pth")
...
def main():
    ...
    for mode in ["Template", "Content"]:
        ...
        dataset = FreqDataset(root); loader = DataLoader(dataset, batch_size=8, shuffle=False, num_workers=4)
        print(f"🚀 Evaluating FreqNet on {mode} Mode...")
        results = []
        with torch.no_grad():
            for imgs, labels, paths in tqdm(loader):
                scores = torch.sigmoid(model(imgs.to(DEVICE)).squeeze(1)).cpu().numpy()
    ...
    report_df = pd.DataFrame(all_metrics); print("\nFREQNET MODEL RESULTS\n" + report_df.to_string(index=False)); report_df.to_csv("results_freqnet_per_manipulation.csv", index=False)
