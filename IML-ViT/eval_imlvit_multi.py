...
CKPT_DIR = os.path.join(REPO_PATH, "checkpoints/IML-ViT_checkpoints")
...
def main():
    ...
    for ckpt_name, ckpt_file in CHECKPOINTS.items():
        print(f"\n🌟 Evaluating Checkpoint: {ckpt_name} ({ckpt_file})")
        ...
        for mode in ["Template", "Content"]:
            ...
            print(f"🚀 Evaluating IML-ViT ({ckpt_name}) on {mode} Mode...")
            results = []
            with torch.no_grad():
        ...
        print(f"\nIML-VIT ({ckpt_name}) RESULTS\n" + report_df.to_string(index=False))
        output_csv = f"results_imlvit_{ckpt_name.lower()}_per_manipulation.csv"
        report_df.to_csv(output_csv, index=False)
        print(f"✅ Saved to {output_csv}")
        
        # Cleanup model to avoid GPU memory issues if any
        del model
